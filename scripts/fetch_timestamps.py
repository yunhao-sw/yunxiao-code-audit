#!/usr/bin/env python3
"""
云效代码审计 - 拉取 commit 时间戳（用于时间/趋势维度）
只拉 commit 元数据，不拉 diff，比 collect.py 快很多。
读取 <workdir>/audit_data.json 获取时间范围和活跃仓库，产出 commit_timestamps.json。

用法:
  python3 fetch_timestamps.py --token <PAT> --org-id <orgId> --workdir <workdir>
"""
import argparse
import json
import time
...
def throttled_request(url, headers, params=None):
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            last_err = e
            wait = 1.5 * (2 ** attempt)  # 指数退避：1.5s, 3s, 6s, 12s
...
def main():
    ...
    print(f"拉取 {len(repo_name_map)} 个活跃仓库的 commit 时间戳...")
    all_commits = {}
