...
"""
云效代码审计 - 数据采集（支持仓库过滤）
在原版 collect.py 基础上增加 --repo-include 参数（逗号分隔的仓库名子串），
只对匹配仓库名子串的仓库采集，便于分项目组输出报告。
"""
import argparse
import requests
import json
...
def match_repo(repo_name, include_patterns, exclude_patterns):
    """仓库名匹配：若 include 为空则全部匹配；任一 include 子串命中即通过；任一 exclude 命中即排除。"""
    if include_patterns:
        ok = any(p.lower() in repo_name.lower() for p in include_patterns)
        if not ok:
            return False
    if exclude_patterns:
        if any(p.lower() in repo_name.lower() for p in exclude_patterns):
            return False
    return True


def collect(base, org_id, token, since, until, out_dir, include=None, exclude=None):
    headers = make_headers(token)
    include_patterns = [p.strip() for p in (include or "").split(",") if p.strip()]
    exclude_patterns = [p.strip() for p in (exclude or "").split(",") if p.strip()]
    print("=" * 60)
    print(f"云效代码审计 - 数据采集（仓库过滤版）")
    print(f"时间范围: {since} → {until}")
    if include_patterns:
        print(f"包含子串: {include_patterns}")
    if exclude_patterns:
        print(f"排除子串: {exclude_patterns}")
    print("=" * 60)
    ...
    # 过滤仓库
    filtered = [r for r in repos if match_repo(r.get('name', ''), include_patterns, exclude_patterns)]
    print(f"  过滤后: {len(filtered)} 个仓库")
    for r in filtered:
        print(f"    · {r.get('name')}")
    ...
    print(f"\n  活跃仓库: {len(active_repos)}/{len(filtered)}")
    ...
    out = {
        ...
        'filtered_repos': [{'id': r['id'], 'name': r['name'], 'description': r.get('description', '')} for r in filtered],
...
if __name__ == "__main__":
    ...
    ap.add_argument('--repo-include', default=None,
                    help='逗号分隔的仓库名子串，仅采集名称包含任一子串的仓库')
    ap.add_argument('--repo-exclude', default=None,
                    help='逗号分隔的仓库名子串，名称包含任一子串则排除')
    args = ap.parse_args()
    ...
    collect(args.base, args.org_id, args.token, since, until, args.out,
            include=args.repo_include, exclude=args.repo_exclude)
    print("\n✅ 采集完成")
